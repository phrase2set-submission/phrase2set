...
public class JavaNodeGroumVisitProcessing {
	...
	public static NodeInfo addNewFieldAccessNode(String varName, String typeVarName, 
		...
		int nodeControlType = NodeInfo.DATA_TYPE;
	...
	public static NodeInfo addVarNode(String varName, String typeName, 
		...
		if (prevControlNode!=null)
			previousControlNodes.add(prevControlNode);
//		previousControlNodes.add(new NodeInfo());
	...
	public static NodeInfo addNewControlNode(MethodInfo curMethodInfo, Stack< NodeInfo> parentNodeStack, 
		...
		{	
			...
			NodeInfo prevNode = previousControlFlowNodeStack.peek();
			previousControlNodes.add(prevNode);
			//			if ((previousControlFlowNodeStack.size()>=2)&&(prevNode.nodeType == NodeInfo.CONTROL_TYPE))
			//				previousControlNodes.add(previousControlFlowNodeStack.get(previousControlFlowNodeStack.size()-2));

			//			if (prevNode.nodeType == NodeInfo.CONTROL_TYPE){
			//				if (prevNode.previousControlNodes!=null)
			//					previousControlNodes.addAll(Arrays.asList(prevNode.previousControlNodes));
			//				if (prevNode.previousDataNodes!=null)
			//					previousDataNodes.addAll(Arrays.asList(prevNode.previousDataNodes));
			//			}
	...
	public static NodeInfo addNewInvocNode(TypeInfo typeInfo, MethodInfo curMethodInfo, 
		...
		{
			NodeInfo prevNode = nodeInfo.previousControlNodes[nodeInfo.previousControlNodes.length-1];
			if (prevNode != null)
//				if (prevNode.nodeType == NodeInfo.CONTROL_TYPE)
				{
					if (prevNode.previousControlNodes!=null)
					{
						if (parentNode!=null){
							if(parentNode.nodeType!=NodeInfo.DATA_TYPE){
								nodeInfo.previousControlNodesTmp.add(parentNode);
							}
						}
						for (NodeInfo node:prevNode.previousControlNodes){
							if (node.nodeType!=NodeInfo.DATA_TYPE)
								nodeInfo.previousControlNodesTmp.add(node);
						}
					}
					if (prevNode.previousDataNodes!=null){
//						previousDataNodes.addAll(Arrays.asList(prevNode.previousDataNodes));
						nodeInfo.previousControlNodesTmp.addAll(Arrays.asList(prevNode.previousDataNodes));
					}
				}
		}
