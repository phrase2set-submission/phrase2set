...
public class Main extends JFrame {
	...
	private final static String VERSION = "0.135";
	...
	Vector<Component> allViews = new Vector<Component>();
	...
	public Main(CrossReferenceServiceConfiguration cfg) {
		...
		selectorsPane = new JTabbedPane(SwingConstants.TOP); // BOTTOM
	...
	progressBar.setFont(progressBar.getFont().deriveFont(9f));
		...
		config.getChangeHistory().addModelUpdateListener(new ModelUpdateListener() {
			...
			public void modelUpdated(EventObject event) {
				...
				Enumeration<Component> enum2 = allViews.elements();
				while (enum2.hasMoreElements()) {
					Component v = enum2.nextElement();
	...
	private Hashtable<Component, JMenuItem> view2item = new Hashtable<Component, JMenuItem>();
	...
	public void closeView(Component view) {
		...
		JMenuItem item = view2item.get(view);
		if (item != null) {
			int i;
			...
			if (i < s) {
				...
				for (s -= 1; i < s; i += 1) {
					...
					char id = next.getText().charAt(0);
					id -= 1;
					next.setText(id + next.getText().substring(1));
					next.setMnemonic(id);
				}
			}
		}
	...
	private final Action openSearchViewAction = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			List<recoder.abstraction.Type> types =  config.getNameInfo().getTypes();
	...
	public List<? extends Reference> getReferences(ProgramModelElement dest) {
		List<? extends Reference> list = null;
		...
		} else if (dest instanceof recoder.abstraction.Type) {
			list = xsi.getReferences((recoder.abstraction.Type)dest);
	...
	JComponent createTypeInfo(ModelElement m) {
		recoder.abstraction.Type t = null;
	...
	JComponent createParameterTypesInfo(Method m) {
		return createElementListProperty("Parameter Types", '\0', "Parameter types of the method or constructor", m, m.getSignature(), "%N", false);
	}

	JComponent createExceptionTypesInfo(Method m) {
		return createElementListProperty("Throws", 'w', "Declared exceptions of a method or constructor", m, m.getExceptions(), "%N", true);
	}
	...
	JComponent createChildrenInfo(NonTerminalProgramElement m) {
		NonTerminalProgramElement nt = m;
	...
	JComponent createExtendsInfo(ClassType m) {
		if (!m.isInterface()) {
			return createElementProperty("Extends", 'e', "Superclass of the class", TypeKit.getSuperClass(config.getNameInfo(), m), "%N");
		} else {
	...
	JComponent createReferersInfo(final ProgramModelElement m) {
		List<? extends Reference> list = getReferences(m);
		return createElementListProperty("Referers", 'r', "All known references to the element", m, list, "%u %4p", "Referers to " + Format.toString("\"%n\"", m), "%u %4p", true);
	}
