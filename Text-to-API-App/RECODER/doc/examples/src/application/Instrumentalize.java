...
/**
 * TODO - conditions of loop statements are not handled correctly: the instrumented code is called outside the loop.
 * @author AL, Tobias Gutzmann
 * @Deprecated - there are open issues
 */
public class Instrumentalize extends Transformation {
    ...
    public ProblemReport execute() {
        Set<MethodReference> inserted = new HashSet<MethodReference>();
        ...
        for (int i = 0; i < units.size(); i += 1, inserted.clear()) {
            ...
            while (tw.next()) {
                ...
                if (pe instanceof MethodReference) {
                    ...
                    if (report != IDENTITY) {
                    	System.out.println("Instrumenting " + caller.getID());
                        t.transform();
                        inserted.add(insert);
                        tw.reset(cu);
                    }
