...
class D {
	...
	class CC extends ArrayList<String> { }
	class DD implements Iterable<String> { public Iterator iterator() { return null; }}
	<T> T CCDD(T a1, T a2) {
		CCDD(new CC(), new DD()).iterator().next().toUpperCase();
		return null;
	}
...
// this is valid code, but recoder used to have problems with it (no longer, works fine now):
class XY<T extends Number> { 
	T getT() { return null; } 
	{ XY<?> a = null; a.getT().byteValue(); }
	{ XY a = null; a.getT().byteValue(); }
}
