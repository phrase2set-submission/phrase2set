 ...
 *
 */
public class CrossValidationWithinProjectRandom {
	public static final String CROSS_VALIDATION_DIR = "/home/dharani/concordia/thesis/SLEMR/CrossValidation";
	public static final String PROJECT_BASE_DIR = "/home/dharani/concordia/thesis/SLEMR/RefactoringMiner/tmp/";
	...
	public CrossValidationWithinProjectRandom(String pn, String gu) {
		...
		// List<List<Graph>> partitions = ListUtils.partition(methodGraphs,
		// methodGraphs.size() / K);
		// System.out.println("partitions " + partitions.size());

		GraphDatabase graphDBTrained = train(methodGraphs);
		List<Graph> randomMethodGraphs = CrossValidationWithinProjectWithRandomMethods.getGraphOfRandomMethods(
				PROJECT_BASE_DIR + File.separator + pn, allClassesRefactored, methodGraphs.size(), graphDB.globalData);
		System.out.println("randomMethodGraphs " + randomMethodGraphs.size());
		testOnValidationSet(graphDBTrained, randomMethodGraphs, 0);
		...
		// System.out.println("Training done. Validation started on " + (i +
		// 1));
	...
	private void testOnValidationSet(GraphDatabase graphDBIn, List<Graph> validationSet, int fold) {
		...
		for (Graph tg : validationSet) {
			...
			// RefactoringGit refG = getRefactoringGit(tg,
			// this.refgVsMethodGraphs);
			//
			...
			if (tg.numNodes() < RefactoredClassesDatabaseBuilder.MIN_NODES) {
				...
				for (Graph testSG : subgraphs) {
					...
					if (result != null) {
						...
						// String dotFile = projectDir + File.separator + fold +
						// FOLD + File.separator + DOTS
						// + File.separator + className + "." + methodName +
						// count++ + ".dot";
						// GraphDrawingUtils.outputDotFile(result, dotFile,
						// graphDB.globalData);
		...
		for (Integer integer : keySet) {
			...
			System.out.println("Number of graphs " + sgWithFrequencies.get(integer).size());
		...
		System.out.println("total sub graphs matched " + numSubGraphsMatched);
	...
	public static void main(String[] args) {
		...
		// new CrossValidationWithinProjectRandom("nodebox",
		// "https://github.com/nodebox/nodebox.git");
		// new CrossValidationWithinProjectRandom("mongo-java-driver",
		// "https://github.com/mongodb/mongo-java-driver.git");

		// new CrossValidationWithinProjectRandom("Activiti",
		// "https://github.com/Activiti/Activiti.git");

		// new CrossValidationWithinProjectRandom("bitcoinj",
		// "https://github.com/bitcoinj/bitcoinj.git");

		new CrossValidationWithinProjectRandom("heritrix3", "https://github.com/internetarchive/heritrix3.git");

		// new CrossValidationWithinProjectRandom("browser-android",
		// "https://github.com/brave/browser-android.git");

		// new CrossValidationWithinProjectRandom("guava",
		// "https://github.com/google/guava.git");

		// new CrossValidationWithinProjectRandom("okhttp",
		// "https://github.com/square/okhttp.git");

		// new CrossValidationWithinProjectRandom("jackson-core",
		// "https://github.com/FasterXML/jackson-core.git");

		// new CrossValidationWithinProjectRandom("jackson-databind",
